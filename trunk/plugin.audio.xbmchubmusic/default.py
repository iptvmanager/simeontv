...
def do_song_search(search_entered,iconimage):
    ...
    url='http://www.mp3chief.com/search?q='+str(search_entered)+'&duration=1&size=1&bitrate=4'
    link = OPEN_URL(url)
...
def for_download_or_playlist(name,artist):
    try:
        ...
        if ADDON.getSetting('bitrate')=='true':
               ...
               r_mp3skull='-->.+?([0-9]*?) kbps<br />[2-9.+?]:.+?<br.+?div style="font-size:15px;">.+?div style="clear:both;"></div>.+?div style="float:left; height:27px; font-size:13px; padding-top:2px;">.+?v style="float:left;"><a href="(.+?)"'
               
               
               
        url='http://mp3skull.com/mp3/'+str(name)+'.html'
        link = OPEN_URL(url).replace('\n','')
        ...
        if url=='':
            url='http://www.mp3chief.com/search?q='+str(name)+'&duration=1&size=1&bitrate=4'
            link = OPEN_URL(url)
...
def album_links(name,artist):
    ...
    url='http://www.mp3chief.com/search?q='+str(name_url)+'&duration=1&size=1&bitrate=4'
    link = OPEN_URL(url)
    match=re.compile('<br>						(.+?) kbps<br>.+?div class="item_name">(.+?)</div>.+?<li class="download"><a href="(.+?)"',re.DOTALL).findall(link)
    print 'MP3 CHIEF========================================='+str(match)
    for bitrate,name,url in match:
...
def song_links(name,url,iconimage,fanart,artist,album):
    if ADDON.getSetting('oneclick')=='true':
        if url=='Number Ones':
            ...
            _artist = artist
    r_mp3skull='-->.+?([0-9]*?) kbps<br />[2-9.+?]:.+?<br.+?div style="font-size:15px;"><b>(.+?)</b></div>.+?div style="clear:both;"></div>.+?div style="float:left; height:27px; font-size:13px; padding-top:2px;">.+?v style="float:left;"><a href="(.+?)"'
    ...
    for bitrate,name,url in match:
        name='[COLOR yellow]MP3 CHIEF[/COLOR]-%sKbps-%s'%(bitrate,name) 
        addLink(name,url,icon2,name_Download,'','','true')    
    url='http://mp3skull.com/mp3/'+str(name_url)+'+'+str(artist_url)+'.html'
    link = OPEN_URL(url)
...
def PLAY_STREAM(name,url,iconimage,artist,album):
    liz=xbmcgui.ListItem(name, iconImage="DefaultVideo.png", thumbnailImage=iconimage)
    liz.setInfo('music', {'Title':name, 'Artist':artist, 'Album':album})
    liz.setProperty('mimetype', 'audio/mpeg')
